...
/// @title AddrResolver
/// @notice Profile mixin for multi-coin address records (ENSIP-9) + the legacy
///         EIP-137 `addr(node) => address` accessor.
/// @dev Ported verbatim-in-intent from
///      reference/ens-contracts/contracts/resolvers/profiles/AddrResolver.sol.
///      The `InvalidEVMAddress(bytes)` custom error is preserved verbatim — ported
///      tests assert this specific revert; the bare-revert posture (D-12) applies
///      to the `authorised(node)` modifier in ResolverBase only.
