...
function horizonRuler(y: number): string {
  ...
  for (let x = x0; x <= x1; x += 26, i++) {
    ...
    ticks += `<line x1="${x}" y1="${y - h}" x2="${x}" y2="${
      y + h
    }" stroke="${col}" stroke-width="${major ? 1.5 : 1}" opacity="${op}"/>`
  }
...
export function renderOg(
  ...
  if (state === 'profile' && opts.bio) {
    subline = `<text x="${W / 2}" y="455" text-anchor="middle"
      font-family="Newsreader" font-style="italic" font-weight="300" font-size="34" fill="${APEX_DIM}">“${
      opts.bio
    }”</text>`
    ...
    subline = `<text x="${W / 2}" y="452" text-anchor="middle"
      ...
      ${
        tier
          ? `<text x="${
              W / 2
            }" y="486" text-anchor="middle" font-family="JetBrains Mono" font-size="15" letter-spacing="3" fill="${APEX_MUTE}">${tier.toUpperCase()}</text>`
          : ''
      }`
  ...
  let footerLeft = ''
  if (state === 'profile') {
    footerLeft = `inscribed at block ${opts.block ?? '12,345,678'} · expires ${
      opts.expires ?? '2027-05-27'
    }`
  } else if (state === 'available') {
  ...
  <g>
    <rect x="${
      W - 200
    }" y="40" width="120" height="38" rx="19" fill="none" stroke="${dot}" stroke-opacity="0.6"/>
    <circle cx="${W - 176}" cy="59" r="5" fill="${dot}"/>
    <text x="${
      W - 160
    }" y="64" font-family="JetBrains Mono" font-size="15" letter-spacing="2" fill="${dot}">${label}</text>
  </g>
  ...
  <!-- name -->
  <text x="${
    avatarX + avatarBox + 40
  }" y="${nameY}" font-family="Newsreader" font-weight="300" font-size="${nameSize}" fill="${APEX}">${stem}<tspan fill="${AMBER}">.</tspan><tspan font-style="italic" fill="url(#rise-${sfx})">${tld}</tspan></text>
  ...
  <text x="${
    W - 80
  }" y="588" text-anchor="end" font-family="JetBrains Mono" font-size="16" letter-spacing="1" fill="${AMBER}">rns.rise</text>
</svg>`
}
