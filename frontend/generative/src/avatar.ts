...
export function describe(name: string): string {
  ...
  return `${COMP_NAMES[p.comp]} · ${GLYPH_NAMES[p.glyph]} · h${p.hue1}/${p.hue2} · ${p.sat}% · rot ${p.rot}°`;
}
...
function background(p: AvatarParams, sfx: string): { defs: string; rect: string } {
  ...
  if (p.comp === 0) {
    ...
    return { defs, rect: `<rect width="${S}" height="${S}" fill="url(#${gid})"/>` };
  }
  if (p.comp === 1) {
...
function glyphMarkup(p: AvatarParams): string {
  ...
  switch (p.glyph) {
    case 0: // sun — disc crossed by a diagonal ray
      body = `<circle cx="${CX}" cy="${CY}" r="${r(0.13)}" fill="${ink}" opacity="0.92"/>
        <line x1="${CX + S * 0.12}" y1="${CY - S * 0.2}" x2="${CX - S * 0.06}" y2="${CY + S * 0.22}"
              stroke="${ink}" stroke-width="${SW}" stroke-linecap="round" opacity="0.88"/>`;
      break;
    case 1: // aperture — concentric rings + core
      body = `<circle cx="${CX}" cy="${CY}" r="${r(0.2)}" fill="none" stroke="${ink}" stroke-width="${SW}" opacity="0.35"/>
        <circle cx="${CX}" cy="${CY}" r="${r(0.13)}" fill="none" stroke="${ink}" stroke-width="${SW}" opacity="0.5"/>
        <circle cx="${CX}" cy="${CY}" r="${r(0.06)}" fill="${ink}" opacity="0.92"/>`;
      break;
    case 2: // orbit — ellipse + core (rot can stand it upright)
      body = `<ellipse cx="${CX}" cy="${CY}" rx="${r(0.22)}" ry="${r(0.11)}" fill="none" stroke="${ink}" stroke-width="${SW}" opacity="0.85"/>
        <circle cx="${CX}" cy="${CY}" r="${r(0.07)}" fill="${ink}" opacity="0.92"/>`;
      break;
    case 3: // arc — hill curve with a dot at the apex
      body = `<path d="M ${CX - S * 0.2} ${CY + S * 0.07} Q ${CX} ${CY - S * 0.21} ${CX + S * 0.2} ${CY + S * 0.07}"
              fill="none" stroke="${ink}" stroke-width="${SW}" stroke-linecap="round" opacity="0.88"/>
        <circle cx="${CX}" cy="${CY - S * 0.085}" r="${r(0.045)}" fill="${ink}" opacity="0.92"/>`;
      break;
    case 4: // diamond — rotated square
      body = `<rect x="${CX - S * 0.15}" y="${CY - S * 0.15}" width="${r(0.3)}" height="${r(0.3)}"
              fill="${ink}" opacity="0.92" transform="rotate(45 ${CX} ${CY})"/>`;
      break;
    default: // 5 chain — three discs, large→small
      body = `<circle cx="${CX - S * 0.16}" cy="${CY}" r="${r(0.085)}" fill="${ink}" opacity="0.92"/>
        <circle cx="${CX + S * 0.0}" cy="${CY}" r="${r(0.06)}" fill="${ink}" opacity="0.92"/>
        <circle cx="${CX + S * 0.14}" cy="${CY}" r="${r(0.04)}" fill="${ink}" opacity="0.92"/>`;
  }
...
export function avatarInner(name: string, sfx = idSuffix(name)): string {
  ...
  return `<defs>
    ${bg.defs}
    <clipPath id="${clip}"><rect width="${S}" height="${S}" rx="${S * 0.22}"/></clipPath>
  </defs>
